...
public class Sword_Skill : Skill
{
    ...
    [SerializeField] private Vector2 laucnhForce;
    ...
    protected override void Update()
    {
        if (Input.GetKeyUp(KeyCode.Mouse1)) 
            finalDir = new Vector2(AimDirection().normalized.x * laucnhForce.x, AimDirection().normalized.y * laucnhForce.y);
    ...
    public Vector2 AimDirection()
    {
        ...
        Vector2 dirrection = mousePosition - playerPosition;
        return dirrection;
    }
    ...
    private Vector2 DotsPosition(float t)
    {
        Vector2 position = (Vector2)player.transform.position +
                            new Vector2(
                                AimDirection().normalized.x * laucnhForce.x,
                                AimDirection().normalized.y * laucnhForce.y)
